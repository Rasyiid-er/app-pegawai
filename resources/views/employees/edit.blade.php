...
@section('content')
    ...
    <h2 class="text-3xl font-semibold tracking-tight sm:text-3xl mb-3 text-center">Edit Data Pegawai</h2>
    ...
    <form action="{{ route('employees.update', $employee->id) }}" method="POST" class="max-w-2xl mx-auto">
        @csrf
        @method('PUT')
        ...
        <div class="relative z-0 w-full mb-5 group">
            <textarea name="alamat" id="alamat" rows="1"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" ">{{ old('alamat', $employee->alamat) }}</textarea>
            <label for="alamat"
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <select name="departemen_id" id="departemen_id" class="w-full p-2 border border-gray-300 rounded">
                    ...
                    @foreach ($departments as $department)
                        <option value="{{ $department->id }}" {{ (old('departemen_id', $employee->departemen_id) == $department->id) ? 'selected' : '' }}>{{ $department->nama_departemen }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <select name="jabatan_id" id="jabatan_id" class="w-full p-2 border border-gray-300 rounded">
                ...
                @foreach ($positions as $position)
                    <option value="{{ $position->id }}" {{ (old('jabatan_id', $employee->jabatan_id) == $position->id) ? 'selected' : '' }}>{{ $position->nama_jabatan }} - {{ $position->gaji_pokok }}</option>
                @endforeach
            </select>
        </div>
