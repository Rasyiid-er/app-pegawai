...
@section('content')
    ...
    <h2 class="text-3xl font-semibold tracking-tight sm:text-3xl mb-3 text-center">Edit Data Absensi</h2>
    ...
    <form action="{{ route('attendance.update', $attendance->id) }}" method="POST" class="max-w-2xl mx-auto">
        @csrf
        @method('PUT')
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <select id="status_absensi" name="status_absensi" class="w-full p-2 border border-gray-300 rounded">
                <option value="hadir" {{ old('status_absensi', $attendance->status_absensi) == 'hadir' ? 'selected' : '' }}>
                    Hadir</option>
                <option value="tidak_hadir" {{ old('status_absensi', $attendance->status_absensi) == 'tidak_hadir' ? 'selected' : '' }}>Tidak Hadir</option>
            </select>
        </div>
